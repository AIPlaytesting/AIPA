...
class DataWriter:
    ...
    def __init__(self, data_collector:AI_Module.TrainingDataCollector.TrainingDataCollector, add_string):
        ...
        self.trainin_data_header = ['Episode #', 'Epsilon', 'Total Cards Played', 'Total Reward', 'Win/Loss', 'Boss End HP', 'Player End HP', 'Max Dmg (1 Turn)', 'Avg Dmg', 'Roll Avg Reward']
    ...
    def WriteFile(self):
        ...
        for row in range(len(self.data_collector.epsilon_list)):
            for col in range(len(self.training_data)):
                if self.trainin_data_header[col] in ['Epsilon', 'Total Reward', 'Avg Dmg', 'Roll Avg Reward']:
        ...
        training_data_worksheet.insert_chart('M2', roll_avg_reward_chart)
