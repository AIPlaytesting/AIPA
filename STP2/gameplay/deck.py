...
class Deck:
    ...
    def reset_deck(self, deck_config):
        ...
        print("deck: ", [str(card_instance)
                         for card_instance in cards_in_deck])
        ...
        for card_instance in self.__cards_on_hand:
            if card_name == card_instance.card_name:
                ...
                for i in range(number_added_to_discard_pile):
                    ...
                    new_card_guid = guid if i == 0 else self.__assign_game_uniqe_id(
                        card_name)
                    self.__discard_pile.addCards(
                        [CardInstance(card_name, new_card_guid)])
                break
